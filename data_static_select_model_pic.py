...
#     """
#     sents, img_sents, labels, img_ids = [], [], [], []
#     with open(data_path, 'r', encoding='UTF-8') as fp:
#         words, labels = [], []
#         for line in fp:
#             line = line.strip()
#             if line != '':
#                 words, tuples, img_id = line.split('####')
#                 img_words = ''
#                 sents.append(words.split())
#                 img_sents.append(img_words.split())
...
#     return sents, img_sents, labels, img_ids
...
#                 at = 'it'
#
#             # one_quad_sentence = f"{ac} is {man_ot} because {at} is {ot}"  ## organization_other is great beacuse [entity] is NULL
#             # one_quad_sentence = f"{at} is a(n) {man_ot} {ac}"  ## can not split the sentimen and category
#             # one_quad_sentence = f"{at} is a(n) {ac}, and is {man_ot}"  ### tooooooooo simple
#
#             # one_quad_sentence = f"{at} is a {ac} and the sentiment towards it is {man_ot}"
#             ## for wiki
#             one_quad_sentence = f"{at} is a {ac.lower()}"
#
...
# def get_para_targets(img_ids, labels, object_detection_faults, use_visual_feats=True, coarse_grained_auxiliary=True):
...
class VisualDataset(Dataset):
    ...
    def __init__(self, tokenizer, data_dir, data_set, data_type, max_len=128, vinvl_region_number=36,
        ...
        # if data_dir == 'wiki':
        #     self.data_path = f'/root/data1/brick/yoli_projects/wikidiverse_ner/t5_format/{data_type}.txt'
...
dataset = VisualDataset(
    ...
    data_set='version10000'
)

breakpoint()

for i in range(len(dataset)):
    if len(torch.nonzero(dataset.targets[0].input_ids == 59)) > 0 and \
            len(torch.nonzero(dataset.targets[i].input_ids == 1023)) > len(
        torch.nonzero(dataset.targets[0].input_ids == 59)):
